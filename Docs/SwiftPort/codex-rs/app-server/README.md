> Swift port note: this file is rewritten for the Swift port while preserving the Rust API contract. Use SwiftPM commands for this repository; Rust paths or Cargo commands appear only when documenting the source behavior in `/Users/mweinbach/Projects/codex`. Check `Docs/PORTING.md` for current Swift completion status.
...
- The Rust app-server uses `RUST_LOG` and `LOG_FORMAT=json`.
- The Swift port should expose equivalent app-server diagnostics through Swift
  logging or structured stderr output when that runtime slice is implemented.
- Keep log fields and user-visible diagnostics compatible with Rust where
...
- `thread/start`, `thread/resume`, and `thread/fork` responses include the legacy `sandbox` compatibility projection. Experimental clients should use `activePermissionProfile` for the named or implicit built-in profile identity/provenance when known; exact runtime permission profiles stay server-side and are no longer returned on lifecycle responses.
...
{ "method": "process/spawn", "id": 40, "params": {
    "command": ["swift", "test", "--filter", "CodexAppServerTests"],
    "processHandle": "swift-app-server-tests-1",
    "cwd": "/Users/me/project",                    // required absolute path
    "env": { "SWIFT_DETERMINISTIC_HASHING": "1" },                    // optional; override or unset app-server env vars
...
6. Regenerate protocol fixtures through the Swift CLI wrapper. The Swift wrapper
   ...
   ```bash
   swift run codex app-server generate-json-schema --out /tmp/codex-app-server-schema
...
7. Verify the Swift app-server and protocol model tests:
   ...
   swift test --filter AppServerProtocolTests
   ```

...
- `codex-rs/app-server/src/request_processors/process_exec_processor.rs` process spawn request validation
  - `process/spawn` now rejects empty commands, empty process handles, relative `cwd`, PTY size without `tty: true`, zero terminal dimensions, and negative `timeoutMs` with Rust-shaped errors. Live process spawning, output streaming, and exit notifications remain pending.
...
- `codex-rs/app-server/src/request_processors/process_exec_processor.rs` process follow-up controls
  - `process/writeStdin`, `process/resizePty`, and `process/kill` now preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, and no-active-process errors. `process/spawn` live process lifecycle remains pending.

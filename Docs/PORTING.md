...
- `codex-rs/app-server/src/request_processors/apps_processor.rs` connector app listing
  - `app/list` now returns the Rust-shaped empty connector page with `data: []` and `nextCursor: null` when no connector catalog is available, rejects invalid or out-of-range cursors, projects configured local/installed plugin app connector entries with app enablement plus Rust-style pagination, and under ChatGPT auth loads connector directory pages from `/connectors/directory/list`, merges hidden/duplicate/blank entries with Rust-style normalization, adds workspace connector catalog entries from `/connectors/directory/list_workspace` for workspace accounts, and applies Rust-style `[apps._default]`, per-app, and requirements-based enablement after connector merging. Live accessible-app auth state, cache/update notifications, and Codex Apps MCP refresh behavior remain pending.

...
- `codex-rs/app-server/src/command_exec.rs` command exec follow-up controls
  - `command/exec` now supports client-supplied `processId` non-PTY sessions with deferred final responses, `streamStdin` writes/close, `streamStdoutStderr` output-delta notifications, duplicate process-id rejection, `command/exec/terminate`, and Rust's timeout exit code; `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, and no-active-process errors. PTY-backed command sessions and active resize remain pending.

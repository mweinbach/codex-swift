...
- `codex-rs/app-server/src/request_processors/external_agent_config_processor.rs` empty external-agent config routes
  - `externalAgentConfig/detect` now returns the Rust-shaped empty detection response when no migration items are discovered, and `externalAgentConfig/import` returns `{}` for an empty migration list. `CONFIG` imports now migrate Claude-style `settings.json` plus `settings.local.json` env/sandbox settings into the appropriate Codex config and emit `externalAgentConfig/import/completed`. Real detection plus sessions/plugins/skills/agents/hooks/commands/MCP imports remain pending.

...
- `codex-rs/model-provider/src/provider.rs` and `codex-rs/model-provider/src/amazon_bedrock/catalog.rs`
  - runtime model-provider abstraction for configured providers and Amazon Bedrock, provider-owned capability dispatch, runtime base-URL lookup, provider auth precedence reuse, ChatGPT-only attestation support detection, Rust-shaped provider account states for unauthenticated/OpenAI API-key/ChatGPT/custom non-OpenAI/Bedrock providers, missing ChatGPT account-detail error text, Bedrock bypass of OpenAI auth, configured Bedrock Mantle region endpoint derivation, unsupported-region rejection text, Bedrock bearer-token auth region requirement, and the default static Bedrock model catalog with no remote refresh/cache writes

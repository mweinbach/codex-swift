...
- `codex-rs/app-server/src/request_processors/external_agent_config_processor.rs` empty external-agent config routes
  - `externalAgentConfig/detect` now reports Rust-shaped `CONFIG`, `SKILLS`, and `AGENTS_MD` migration items for Claude-style settings/skills/instructions that would add missing Codex config values, skill directories, or `AGENTS.md`, and still returns an empty detection response when no migration items are discovered. `externalAgentConfig/import` returns `{}` for an empty migration list. `CONFIG` imports now migrate Claude-style `settings.json` plus `settings.local.json` env/sandbox settings into the appropriate Codex config, `SKILLS` imports copy missing skill directories and rewrite Claude-specific terms in `SKILL.md`, and `AGENTS_MD` imports rewrite/copy non-empty `CLAUDE.md` sources into empty or missing `AGENTS.md`; all emit `externalAgentConfig/import/completed`. Sessions/plugins/hooks/commands/MCP imports remain pending.

...
- Initial `codex-rs/core/src/tools/runtimes/apply_patch.rs` live tool execution
  - non-interactive Responses loops now treat JSON/function and freeform `custom_tool_call` `apply_patch` items as runnable, apply patches under the existing Swift patch engine, return matching function/custom-tool outputs, and reject read-only sandbox writes when approval policy is `never`

...
- `codex-rs/app-server-protocol/src/protocol/v2/turn.rs` turn-start context override enum decoding
  - `turn/start` now rejects unknown `approvalPolicy`, `effort`, and `summary` context override enum values before recording input or emitting `turn/started`, matching Rust serde validation instead of silently dropping malformed overrides. Full live runtime override dispatch for approvals reviewer, permissions, service tier, collaboration mode, and personality remains pending with the broader running-thread runtime.

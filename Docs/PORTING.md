...
- `codex-rs/app-server/src/request_processors/fs_processor.rs` filesystem methods
  - `fs/readFile`, `fs/writeFile`, `fs/createDirectory`, `fs/getMetadata`, `fs/readDirectory`, `fs/remove`, and `fs/copy` now validate absolute paths, preserve Rust response shapes and default recursive/force behavior, base64 encode/decode file bytes, report symlink-aware metadata, reject directory copies without `recursive: true`, prevent recursive self/descendant copies, and preserve symlinks in recursive copies. `fs/watch` and `fs/unwatch` remain pending as a separate eventing slice.

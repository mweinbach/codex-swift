...
- Initial hook command-output parsing from `codex-rs/hooks/src/engine/output_parser.rs`
  - compact hook output parsers now preserve Rust's universal `continue`/`stopReason`/`suppressOutput`/`systemMessage` defaults, object-only JSON parsing, `looks_like_json` heuristic, and serde-style unknown-field rejection for pre/post compact command outputs. Permission request output parsing now preserves allow/deny decisions, trimmed/default deny messages, unsupported universal/reserved-field invalid reasons, and serde-style malformed-shape rejection. PreToolUse output parsing now preserves legacy block/reason behavior, hook-specific deny/additional-context precedence, unsupported universal/legacy/hook-specific invalid reasons, and malformed-shape rejection. PostToolUse output parsing now preserves block/reason validation, continue/stopReason allowance, unsupported suppressOutput and updatedMCPToolOutput invalid reasons, additional context, and malformed-shape rejection. UserPromptSubmit and Stop output parsing now preserve Rust's block/reason validation, reason-without-decision tolerance, universal-field handling, additional context for prompt submit, and malformed-shape rejection. SessionStart output parsing now preserves universal fields, additional context, and malformed-shape rejection. App-server hook discovery now projects user, plugin, project, and managed requirement command hooks, per-cwd project hook layers, plugin/managed hook load warnings, and hook state enablement/trust metadata for list/config writes. Hook execution and output spilling remain pending.

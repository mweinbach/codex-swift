...
- `codex-rs/app-server/src/bespoke_event_handling.rs` MCP startup status notifications
  - Runtime `McpStartupUpdate` events now emit Rust-shaped `mcpServer/startupStatus/updated` app-server notifications with `name`, `status`, and nullable `error` for starting, ready, failed, and cancelled startup states. Wiring real app-server thread startup into the live MCP connection manager remains pending with the broader running-thread runtime.

...
- Initial `codex-rs/app-server` rollout-listing surface
  - executable `codex app-server` now runs a Swift-native newline-delimited app-server JSON-RPC loop for initialize, `getUserAgent`, v2 `thread/list`, v2 `thread/archive`, v2 `skills/list`, v2 `model/list`, v2 `mcpServerStatus/list`, v2 `account/read`, v2 `account/login/start` API-key flow, v2 `account/login/cancel`, v2 `account/logout`, v2 `feedback/upload`, v2 `config/read`, v2 `config/value/write`, v2 `config/batchWrite`, v2 `command/exec`, legacy `getUserSavedConfig`, legacy `gitDiffToRemote`, legacy `fuzzyFileSearch`, legacy `execOneOffCommand`, legacy `archiveConversation`, legacy `loginApiKey`, legacy `logoutChatGpt`, legacy `setDefaultModel`, legacy `getAuthStatus`, legacy `userInfo`, and legacy `listConversations`, backed by saved rollout files with provider filtering, pagination cursors, preview extraction, session source mapping, archive moves, model picker data, MCP server status pagination/auth projection, feedback upload envelopes, skill discovery projection, fuzzy file-search projection, one-off command execution projection, auth/account status projection, legacy and v2 auth mutation notifications, effective config projection, config-layer origins, config write projection, saved config projection, git diff-to-remote projection, model-selection persistence, and thread/conversation response shapes

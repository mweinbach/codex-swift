...
- `codex-rs/app-server/src/request_processors/external_agent_config_processor.rs` empty external-agent config routes
  - `externalAgentConfig/detect` now returns the Rust-shaped empty detection response when no migration items are discovered, and `externalAgentConfig/import` returns `{}` for an empty migration list. Real config/session/plugin detection and non-empty imports remain pending.

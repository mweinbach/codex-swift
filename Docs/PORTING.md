...
- `codex-rs/app-server/src/command_exec.rs` command exec follow-up controls
  - `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` now preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, and no-active-process errors. Live streamed command sessions remain pending.

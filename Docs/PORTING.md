...
- 2026-05-15: rechecked Rust's accepted-line repository URL canonicalization in `codex-rs/git-utils/src/info.rs`. Swift now has focused coverage that URL-like Git remotes strip query/fragment suffixes, normalize default `git` / `http` ports, preserve the Rust exact scheme allowlist, and reject `.` / `..` owner or repo path components before accepted-line repo hashing.

...
- `codex-rs/exec-server/src/client/reqwest_http_client.rs` and `codex-rs/exec-server/tests/http_request.rs`
  - Swift now routes initialized buffered `http/request` calls through an injectable HTTP client, including Rust-shaped HTTP/HTTPS URL validation, method/header validation, request body and timeout propagation, response status/header/body mapping, and JSON-RPC error envelopes for validation and transport failures. Streamed response bodies via `http/request/bodyDelta`, duplicate streamed request-id reservation, and remote HTTP transport remain pending.

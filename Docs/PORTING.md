...
- `codex-rs/app-server/src/request_processors/process_exec_processor.rs` process spawn request validation
  - `process/spawn` now rejects empty commands, empty process handles, relative `cwd`, PTY size without `tty: true`, zero terminal dimensions, and negative `timeoutMs` with Rust-shaped errors, starts non-PTY host processes for valid requests, supports `streamStdin` pipe writes, reports Rust's timeout exit code, and emits Rust-shaped `process/outputDelta` / `process/exited` notifications for buffered and streamed output. `tty: true` spawned processes now attach to a Darwin pseudo-terminal and accept active `process/resizePty` calls; broader PTY output/control parity remains pending.

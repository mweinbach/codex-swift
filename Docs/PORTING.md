...
- `codex-rs/exec-server/src/server/transport.rs`, `remote.rs`, and CLI `exec-server`
  - `codex exec-server` now parses Rust's `--listen`, `--remote`, `--executor-id`, and `--name` flags, rejects root remote-mode flags, enforces `--listen`/`--remote` conflicts and remote executor-id requirements, validates `stdio`/`stdio://`/`ws://IP:PORT` listen URLs, preserves remote executor base URL, executor id, name, and bearer-token normalization errors, and runs the `--listen stdio` transport over process stdin/stdout. WebSocket listening and remote executor registration remain pending.
...
- `codex-rs/exec-server/src/server/processor.rs` in-memory connection lifecycle
  - Swift now mirrors the processor's sequential event handling around the ported router: malformed messages produce request id `-1` invalid-request errors without closing, requests route through the Rust-shaped router, expected `initialized` notifications are accepted, unexpected notifications/responses/errors close the connection, disconnect shuts down and detaches the session, detached sessions can be resumed by a later connection, and the stdio transport keeps separate inbound and wakeable outbound paths so queued async notifications can be written without waiting for another input line. The WebSocket listener remains pending.
- `codex-rs/exec-server/src/connection.rs` JSON-RPC message codec
  - Swift now mirrors the newline stdio and websocket JSON-RPC codec boundary used by the exec-server connection: blank stdio lines are ignored, valid stdio/websocket text/binary payloads decode into processor events, malformed payloads become labeled malformed-message events, outbound JSON-RPC messages encode as compact websocket text or newline-terminated stdio data, and the stdio transport composes that codec with process stdin/stdout plus the ported connection processor. Child-process supervision, disconnect watch channels, and live WebSocket serving remain pending.

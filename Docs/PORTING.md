...
- `codex-rs/app-server/src/request_processors/plugins.rs` empty plugin listing
  - `plugin/list` now accepts Rust's removed `forceRemoteSync` field, validates absolute `cwds` and known `marketplaceKinds`, and returns the Rust-shaped empty plugin catalog response (`marketplaces`, `marketplaceLoadErrors`, `featuredPluginIds`) when plugin catalogs are unavailable. Local marketplace loading, remote catalogs, featured IDs, and plugin cache warmup remain pending.

...
- Initial `codex-rs/app-server` rollout-listing surface
  ...
  - `command/exec` and `process/spawn` now reject negative or wrongly typed `outputBytesCap` and wrongly typed `timeoutMs` values with Rust's numeric protocol decode errors instead of silently clamping or defaulting them.

...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` thread status notifications
  - `turn/start` now emits Rust-shaped `thread/status/changed` active notifications after the rollout-backed `turn/started` notification, `turn/interrupt` emits idle status after the interrupted completion notification, and runtime `task_started`, every runtime `error`, `task_complete`, and `turn_aborted` event emits active, system-error, and idle status transitions matching Rust's running-thread manager. Initialize `optOutNotificationMethods` suppresses those notifications. Broader approval/user-input active flags and unloaded transitions remain pending with the running-thread runtime.

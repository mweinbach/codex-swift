...
- Initial `codex-rs/lmstudio/src/client.rs` provider probing
  - LM Studio clients now probe `/models`, preserve Rust's missing-base-url and connection/server error text, parse `/models` response `data[].id` model IDs, surface missing-data and failed-fetch errors with Rust strings, POST the minimal `/responses` model-load request with `input: ""` and `max_output_tokens: 1`, discover the `lms` CLI via PATH or `$HOME/.lmstudio/bin/lms`, print the Rust download banner, execute the Rust-matching `lms get --yes {model}` command with stdout inherited and stderr discarded, and preserve the download spawn/nonzero-exit failure strings. Runtime startup wiring remains pending.
...
- `codex-rs/app-server/src/command_exec.rs` command exec follow-up controls
  - `command/exec` now supports buffered and client-supplied `processId` non-PTY sessions with Rust's timeout exit code, deferred final responses for process-id sessions, `streamStdin` writes/close, live `streamStdoutStderr` output-delta notifications, duplicate process-id rejection, and `command/exec/terminate`; `command/exec` rejects Rust option conflicts for `permissionProfile` with `sandboxPolicy`, `disableOutputCap` with `outputBytesCap`, and `disableTimeout` with `timeoutMs`; `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, no-active-process errors, and active non-PTY resize failures. `tty: true` command sessions now attach to a Darwin pseudo-terminal, accept active `command/exec/resize`, force output through Rust-shaped delta notifications, and leave final `command/exec` output empty.

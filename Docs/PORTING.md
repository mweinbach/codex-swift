...
- `codex-rs/core/src/session/session.rs`, `codex-rs/exec/tests/suite/mcp_required_exit.rs`, `codex-rs/app-server/tests/suite/v2/thread_start.rs`, `codex-rs/app-server/tests/suite/v2/thread_resume.rs`, and `codex-rs/app-server/src/request_processors/turn_processor.rs` required MCP startup failures and approval-reviewer overrides
  - `thread/start`, `thread/resume`, `thread/fork`, and non-interactive exec now reject enabled required stdio MCP servers whose command cannot be resolved before creating or reopening a rollout, preserving Rust's sorted `required MCP servers failed to initialize: <server>: <error>` failure shape for broken-command startup. Loaded Swift app-server threads now also start enabled MCP servers through the live MCP manager slice, preserve tool inventory for optional servers, ignore optional startup failures, and wait only on required MCP failures like Rust's session connection manager. Thread lifecycle and `turn/start` approval-reviewer overrides now reject unknown enum values instead of falling back to the configured reviewer. MCP lifecycle notifications and running-thread tool routing remain pending with the broader MCP manager port.

...
- `codex-rs/app-server/src/request_processors/marketplace_processor.rs` empty marketplace upgrades
  - `marketplace/upgrade` now returns Rust's empty upgrade response when no Git marketplaces are configured, and rejects a requested marketplace that is absent or only locally configured with Rust's invalid-request message. Git clone/upgrade, cache refresh, add, and remove flows remain pending.

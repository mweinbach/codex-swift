...
- `codex-rs/exec-server/src/server/processor.rs` in-memory connection lifecycle
  - Swift now mirrors the processor's sequential event handling around the ported router: malformed messages produce request id `-1` invalid-request errors without closing, requests route through the Rust-shaped router, expected `initialized` notifications are accepted, unexpected notifications/responses/errors close the connection, disconnect shuts down and detaches the session, detached sessions can be resumed by a later connection, and the stdio/WebSocket transports keep separate inbound and wakeable outbound paths so queued async notifications can be written without waiting for another input message.
- `codex-rs/exec-server/src/connection.rs` JSON-RPC message codec
  - Swift now mirrors the newline stdio and websocket JSON-RPC codec boundary used by the exec-server connection: blank stdio lines are ignored, valid stdio/websocket text/binary payloads decode into processor events, malformed payloads become labeled malformed-message events, outbound JSON-RPC messages encode as compact websocket text or newline-terminated stdio data, and the stdio/WebSocket transports compose that codec with process stdin/stdout or accepted websocket clients plus the ported connection processor. Child-process supervision and disconnect watch channels remain pending.

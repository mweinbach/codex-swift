...
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled plugin share/install/uninstall routes
  - `plugin/share/save`, `plugin/share/updateTargets`, `plugin/share/list`, and `plugin/share/delete` now preserve Rust's disabled sharing response when the plugins feature is unavailable; `plugin/install` validates exactly one install source, materializes local marketplace plugin sources into the local plugin cache, enables `[plugins."<id>"]`, and returns the Rust install response shape; `plugin/uninstall` accepts Rust's removed `forceRemoteSync` field, rejects malformed ids, removes local plugin cache/config state, and returns the remote disabled-feature error for remote plugin ids. Remote install/uninstall mutations and app-auth follow-up prompts remain pending.

...
- Initial `codex-rs/cli/src/debug_sandbox.rs` macOS sandbox execution
  - executable `codex sandbox macos`/`seatbelt` now runs commands through `/usr/bin/sandbox-exec` with Rust-shaped read-only, explicit `sandbox_mode` workspace-write, and built-in `--permissions-profile :read-only|:workspace|:danger-no-sandbox` Seatbelt policies, honors `-C`/`--cd` for the command and sandbox policy cwd, preserves inherited stdio, `CODEX_SANDBOX`/network-disabled environment markers, and optional denial-log summaries. The CLI surface now matches Rust's current `codex sandbox macos|linux|windows` profile controls, including `--permissions-profile`, `-C`/`--cd`, `--include-managed-config`, macOS `--allow-unix-socket`, and rejection of the removed sandbox `--full-auto` flag; custom `[permissions]` profile compilation, managed requirements, and Unix socket allowlist runtime grants remain pending.

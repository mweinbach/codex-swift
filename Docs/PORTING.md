...
- Initial `codex-rs/cli/src/debug_sandbox.rs` macOS sandbox execution
  - executable `codex sandbox macos`/`seatbelt` now runs commands through `/usr/bin/sandbox-exec` with Rust-shaped read-only, explicit legacy `sandbox_mode` workspace-write, and built-in `--permissions-profile :read-only|:workspace|:danger-no-sandbox` Seatbelt policies, keeps ambient legacy sandbox configs read-only unless `sandbox_mode` is explicitly overridden, carries configured `sandbox_workspace_write` roots/network/tmp flags into the explicit `:workspace` profile, honors `-C`/`--cd` for the command and sandbox policy cwd, preserves inherited stdio, `CODEX_SANDBOX`/network-disabled environment markers, and optional denial-log summaries. The CLI surface now matches Rust's current `codex sandbox macos|linux|windows` profile controls, including `--permissions-profile`, `-C`/`--cd`, `--include-managed-config`, macOS `--allow-unix-socket`, and rejection of the removed sandbox `--full-auto` flag; custom `[permissions]` profile compilation, managed requirements, and Unix socket allowlist runtime grants remain pending.

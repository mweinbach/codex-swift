...
- `codex-rs/app-server/src/request_processors/external_agent_config_processor.rs` empty external-agent config routes
  - `externalAgentConfig/detect` now reports Rust-shaped `CONFIG` migration items for Claude-style settings that would add missing Codex config values, and still returns an empty detection response when no migration items are discovered. `externalAgentConfig/import` returns `{}` for an empty migration list. `CONFIG` imports now migrate Claude-style `settings.json` plus `settings.local.json` env/sandbox settings into the appropriate Codex config and emit `externalAgentConfig/import/completed`. Sessions/plugins/skills/agents/hooks/commands/MCP imports remain pending.

...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` thread list state DB mode
  - `thread/list` now honors `useStateDbOnly` when a Swift SQLite state store is configured, returning state metadata without scanning JSONL rollouts, dropping/deleting stale SQLite rows whose rollout files are missing, and preserving Rust's empty page fallback when no state store is available. Default non-state-only listing now warms the configured SQLite state store from filesystem hits so a later `useStateDbOnly` listing can see repaired rollout metadata, repairs stale state DB search hits before returning filesystem-backed search pages, and overlays state DB Git metadata when the rollout head lacks it; broader DB-page-first unfiltered listing remains pending for the full state DB/runtime integration.

...
- `codex-rs/core/src/tools/handlers/agent_jobs.rs` runtime helper layer
  - added Swift concurrency/runtime normalization, worker prompt rendering, spawn-result/failure-summary wire models, `report_agent_job_result` result wire shape, `spawn_agents_on_csv` job preparation, missing-output CSV snapshot export with parent-directory creation, final result assembly with the first five failed item summaries, and runner recovery/finalization helpers for stale running items, malformed assigned thread IDs, final worker status, and active item timeout reaping. The actual subagent spawn loop, live state-db session plumbing, worker stop propagation beyond store cancellation, and tool-handler registration remain pending.

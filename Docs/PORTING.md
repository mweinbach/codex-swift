...
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled remote plugin detail reads
  - `plugin/read` now preserves Rust's exactly-one-source validation for `marketplacePath` vs. `remoteMarketplaceName`, validates absolute local marketplace paths, returns Rust-shaped local plugin detail for local marketplace manifests, hydrates bundled `skills/` plus top-level manifest `skills`, manifest/default hook config files when `plugin_hooks` is enabled, manifest/default app connector summaries, and manifest/default MCP server names, and returns the Rust disabled-feature error for remote plugin reads; `plugin/skill/read` returns the matching disabled-feature error before remote plugin-id or skill-name validation. Inline manifest hooks, remote catalog fetches, and live connector auth metadata remain pending.

...
- `codex-rs/cli/src/main.rs` debug models bundled runtime
  - executable `codex debug models --bundled` now emits the Swift bundled model catalog as JSON through the debug command runtime. `codex debug models` now loads the effective config/auth, reads a fresh version-matched `models_cache.json` when available, fetches `/models?client_version=...` when command auth or the Codex backend permits refresh, persists the returned cache with `etag` and `client_version`, merges remote entries over bundled models like Rust, and falls back to the bundled catalog on refresh failure.
...
- `codex-rs/exec-server/src/environment_{provider,toml}.rs` and `codex-rs/core/src/environment_selection.rs`
  - `$CODEX_HOME/environments.toml` loading, config-path inspect error wrapping, legacy `CODEX_EXEC_SERVER_URL` fallback, environment id/default validation, websocket and stdio environment entries, relative stdio cwd resolution, and default-first turn environment selections
...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` thread list state DB mode
  - `thread/list` now honors `useStateDbOnly` when a Swift SQLite state store is configured, returning state metadata without scanning JSONL rollouts, dropping/deleting stale SQLite rows whose rollout files are missing, and preserving Rust's empty page fallback when no state store is available. Default non-state-only listing now warms the configured SQLite state store from filesystem hits so a later `useStateDbOnly` listing can see repaired rollout metadata, repairs stale state DB hits for explicit metadata filters before returning filesystem-backed pages, overlays state DB Git metadata when the rollout head lacks it, and returns the DB page for unfiltered listings after the repair pass.
...
- `codex-rs/app-server/src/request_processors/config_processor.rs` config writes
  - `config/value/write` and `config/batchWrite` preserve Rust user-config write response/error shapes for replacement writes, batch writes, version conflicts, readonly-layer rejection, unknown merge-strategy rejection, pipelined read-after-write visibility, null-clearing missing paths as successful no-ops, replace/upsert table merge semantics, pre-write validation of edited user/effective config, and reserved built-in model-provider ID rejection. Runtime hot-reload into loaded live threads remains pending with the broader live-thread runtime.

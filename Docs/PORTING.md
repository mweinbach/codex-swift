...
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled remote plugin detail reads
  - `plugin/read` now preserves Rust's exactly-one-source validation for `marketplacePath` vs. `remoteMarketplaceName`, validates absolute local marketplace paths, returns Rust-shaped local plugin detail for local marketplace manifests, hydrates bundled `skills/`, `hooks/hooks.json` when `plugin_hooks` is enabled, `.app.json` connector summaries, and `.mcp.json` server names, and returns the Rust disabled-feature error for remote plugin reads; `plugin/skill/read` returns the matching disabled-feature error before remote plugin-id or skill-name validation. Manifest-declared alternate capability paths, remote catalog fetches, and live connector auth metadata remain pending.

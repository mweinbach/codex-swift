...
- `codex-rs/app-server/src/request_processors/plugins.rs` empty plugin listing
  - `plugin/list` now accepts Rust's removed `forceRemoteSync` field, validates absolute `cwds` and known `marketplaceKinds`, returns the Rust-shaped empty plugin catalog response (`marketplaces`, `marketplaceLoadErrors`, `featuredPluginIds`) when plugin catalogs are unavailable, and projects configured/local marketplace manifests with local plugin summaries, interface metadata, policies, keywords, enabled-state config, and local install-state cache detection. Remote catalogs, featured IDs, and plugin cache warmup remain pending.
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled remote plugin detail reads
  - `plugin/read` now preserves Rust's exactly-one-source validation for `marketplacePath` vs. `remoteMarketplaceName`, validates absolute local marketplace paths, returns Rust-shaped local plugin detail for local marketplace manifests, hydrates bundled `skills/` plus top-level manifest `skills`, manifest/default hook config files and inline manifest hooks when `plugin_hooks` is enabled, manifest/default app connector summaries, and manifest/default MCP server names, and returns the Rust disabled-feature error for remote plugin reads; `plugin/skill/read` returns the matching disabled-feature error before remote plugin-id or skill-name validation. Remote catalog fetches and live connector auth metadata remain pending.

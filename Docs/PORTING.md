...
- `codex-rs/exec-server/src/server/processor.rs` in-memory connection lifecycle
  - Swift now mirrors the processor's sequential event handling around the ported router: malformed messages produce request id `-1` invalid-request errors without closing, requests route through the Rust-shaped router, expected `initialized` notifications are accepted, unexpected notifications/responses/errors close the connection, disconnect shuts down and detaches the session, detached sessions can be resumed by a later connection, and the line-oriented stdio server helper drains direct responses plus queued async notifications through the same newline-delimited JSON-RPC shape Rust writes. Live process stdin/stdout binding and the WebSocket listener remain pending.
- `codex-rs/exec-server/src/connection.rs` JSON-RPC message codec
  - Swift now mirrors the newline stdio and websocket JSON-RPC codec boundary used by the exec-server connection: blank stdio lines are ignored, valid stdio/websocket text/binary payloads decode into processor events, malformed payloads become labeled malformed-message events, outbound JSON-RPC messages encode as compact websocket text or newline-terminated stdio data, and a stdio line-server facade composes that codec with the ported connection processor. Child-process supervision, process-global async IO task spawning, disconnect watch channels, and live WebSocket serving remain pending.

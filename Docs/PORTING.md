...
- `codex-rs/core/src/tools/handlers/agent_jobs.rs` runtime helper layer
  - added Swift concurrency/runtime normalization, worker prompt rendering, spawn-result/failure-summary wire models, `report_agent_job_result` result wire shape, and the front half of `spawn_agents_on_csv` job preparation: argument decoding, instruction/CSV validation, ID-column item construction, output-path derivation, job creation, running transition, max-runtime selection, and concurrency normalization. The actual subagent runner loop, live state-db session plumbing, worker stop propagation beyond store cancellation, and tool-handler registration remain pending.

...
- `codex-rs/app-server/src/request_processors/apps_processor.rs` connector app listing
  - `app/list` now returns the Rust-shaped empty connector page with `data: []` and `nextCursor: null` when no connector catalog is available, rejects invalid or out-of-range cursors, and projects configured local/installed plugin app connector entries with app enablement plus Rust-style pagination. Live remote connector/app catalog loading and accessible-app auth state remain pending.

...
- `codex-rs/cli/src/main.rs` debug prompt-input runtime
  - executable `codex debug prompt-input` now emits a pretty-printed JSON model input list with Rust-ordered initial permissions instructions and environment context, honors the matching config gates, converts local images, includes optional prompt text, preserves Rust's CRLF/CR prompt normalization, and keeps Rust's root-level image then subcommand image ordering. Full Rust session startup side effects, tool router construction, skills injection, and configured environment expansion remain pending.

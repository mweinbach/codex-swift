...
- `codex-rs/app-server/src/request_processors/thread_goal_processor.rs` disabled goal route gate
  - `thread/goal/set`, `thread/goal/get`, and `thread/goal/clear` now return Rust's default `goals feature is disabled` invalid-request error when the experimental goals feature is off. Full enabled goal persistence, notifications, and resume continuation remain pending.

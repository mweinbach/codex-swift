...
- Initial `codex-rs/app-server` rollout-listing surface
  ...
  - `command/exec` and `process/spawn` now reject negative `outputBytesCap` values with Rust's unsigned-protocol decode error instead of silently clamping them to a zero-byte cap.

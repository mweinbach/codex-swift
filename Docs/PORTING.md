...
- `codex-rs/app-server/src/request_processors/catalog_processor.rs` hook listing
  - `hooks/list` now returns Rust-shaped entries for user-configured command hooks from `config.toml`, including event/matcher/handler metadata, feature-disable behavior, source paths, hash placeholders, trust status, and Rust's configured-session-cwd default when `cwds` is omitted.

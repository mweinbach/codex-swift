...
- `codex-rs/core/src/session/session.rs`, `codex-rs/exec/tests/suite/mcp_required_exit.rs`, and `codex-rs/app-server/tests/suite/v2/thread_start.rs` required MCP startup failures
  - `thread/start` and non-interactive exec now reject enabled required stdio MCP servers whose command cannot be resolved before creating a rollout, preserving Rust's sorted `required MCP servers failed to initialize: <server>: <error>` failure shape for broken-command startup. Full live MCP connection-manager startup and runtime failure waiting remains pending with the broader MCP manager port.

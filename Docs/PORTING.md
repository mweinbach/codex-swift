...
- `codex-rs/app-server-protocol/src/protocol/v2/turn.rs` turn-start context override enum decoding
  - `turn/start` now rejects unknown `approvalPolicy`, `effort`, `summary`, and `personality` context override enum values before recording input or emitting `turn/started`, matching Rust serde validation instead of silently dropping malformed overrides. Rollout-backed turn contexts now persist supplied `personality` and `outputSchema` as Rust's `personality` and `final_output_json_schema` fields. Experimental `collaborationMode` requests now decode Rust's typed settings, normalize `null` developer instructions to the built-in mode text, and persist the resulting collaboration mode with its model/effort precedence in the turn context. Full live runtime override dispatch for approvals reviewer, permissions, service tier, and personality remains pending with the broader running-thread runtime.

...
- Initial `codex-rs/ollama/src/client.rs` provider probing
  - Ollama clients now derive the native host root from OpenAI-compatible `/v1` provider URLs, probe `/v1/models` or `/api/tags` according to the provider URL shape, parse `/api/tags` model names, parse optional `/api/version` semver responses, treat unavailable or unparsable versions as allowed, and reject too-old Responses API versions with Rust's error text. Streaming `/api/pull` download readiness remains pending.
...
- LM Studio provider clients, Ollama streaming model download readiness, and local-provider runtime wiring

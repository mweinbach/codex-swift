...
- `codex-rs/app-server-protocol/src/protocol/common.rs`, `codex-rs/app-server-protocol/src/protocol/v1.rs`, `codex-rs/app-server-protocol/src/protocol/v2/attestation.rs`, and `codex-rs/app-server-protocol/src/protocol/v2/account.rs`
  - server-initiated `attestation/generate`, `account/chatgptAuthTokens/refresh`, and legacy `execCommandApproval` request/response protocol envelopes, empty attestation params shape, token and approval response shapes, request id preservation, method-tag validation, and request-payload-to-request helper behavior

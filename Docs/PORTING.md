...
- `codex-rs/app-server/src/request_processors/marketplace_processor.rs` empty marketplace upgrades
  - `marketplace/upgrade` now returns Rust's empty upgrade response when no Git marketplaces are configured, and rejects a requested marketplace that is absent or only locally configured with Rust's invalid-request message. Git upgrade and cache refresh remain pending.
...
- `codex-rs/app-server/src/request_processors/marketplace_processor.rs` marketplace removal
  - `marketplace/remove` now validates marketplace-name segments, rejects unknown and case-mismatched marketplace names with Rust messages, removes user config entries, removes installed marketplace roots under `.tmp/marketplaces`, and returns the Rust-shaped nullable `installedRoot`. Git upgrade flows remain pending.

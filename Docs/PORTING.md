...
- `codex-rs/app-server/src/request_processors/catalog_processor.rs` hook listing
  - `hooks/list` now returns Rust-shaped entries for user-configured command hooks from `config.toml` and enabled installed local plugin command hooks, including event/matcher/handler metadata, feature-disable behavior, source paths, hash placeholders, trust status, plugin hook load warnings, and Rust's configured-session-cwd default when `cwds` is omitted.

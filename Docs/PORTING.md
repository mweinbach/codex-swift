...
- `codex-rs/exec-server/src/server/process_handler.rs` and active-connection pipe-backed `local_process.rs`
  - Swift now routes initialized exec-server process start/read/write/terminate requests through a local process table for non-PTY child processes, including Rust-shaped argv validation, duplicate/unknown process errors, closed/unknown stdin write statuses, environment-policy overlay, retained output chunks with sequence numbers, exit/closed state, read wait windows, and terminate responses. PTY execution, process output notifications, exited-process retention cleanup, remote process transport, and preserving live processes across session resume remain pending.

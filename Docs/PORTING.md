...
- `codex-rs/builtin-mcps/src/lib.rs` and `codex-rs/core/src/config/mod.rs` built-in MCP selection
  - added the Swift product-owned built-in MCP catalog for the `memories` server, preserving Rust's server name, parallel-call support, non-polluting memory metadata, current feature-gated enablement, and runtime MCP config behavior that removes a user-configured `memories` server when the built-in server is active. Rust's additional `memories.use_memories` gate and full live connection-manager spawning of built-in MCP servers remain pending.

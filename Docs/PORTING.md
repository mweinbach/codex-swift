...
- `codex-rs/app-server/src/command_exec.rs` command exec follow-up controls
  - `command/exec` now supports buffered and client-supplied `processId` non-PTY sessions with Rust's timeout exit code, deferred final responses for process-id sessions, `streamStdin` writes/close, `streamStdoutStderr` output-delta notifications, duplicate process-id rejection, and `command/exec/terminate`; `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, and no-active-process errors. PTY-backed command sessions and active resize remain pending.
- `codex-rs/app-server/src/request_processors/process_exec_processor.rs` process follow-up controls
  - `process/writeStdin`, `process/resizePty`, and `process/kill` now preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, and no-active-process errors; `process/writeStdin` writes to active `streamStdin` processes and preserves Rust's disabled-stdin error, while `process/kill` can terminate active non-PTY spawned processes. Active-process PTY resize control remains pending.

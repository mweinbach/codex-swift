...
- Initial `codex-rs/cli/src/debug_sandbox.rs` macOS sandbox execution
  - executable `codex sandbox macos`/`seatbelt` now runs commands through `/usr/bin/sandbox-exec` with Rust-shaped read-only and explicit `sandbox_mode` workspace-write Seatbelt policies, inherited stdio, `CODEX_SANDBOX`/network-disabled environment markers, and optional denial-log summaries. The CLI surface now matches Rust's current `codex sandbox macos|linux|windows` profile controls, including `--permissions-profile`, `-C`/`--cd`, `--include-managed-config`, macOS `--allow-unix-socket`, and rejection of the removed sandbox `--full-auto` flag; resolving explicit permission profiles and managed requirements in the sandbox runtime remains pending.

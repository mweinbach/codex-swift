...
- `codex-rs/cli/src/main.rs` debug prompt-input runtime
  - executable `codex debug prompt-input` now emits a pretty-printed JSON model input list with environment context, local image conversion, optional prompt text, Rust's CRLF/CR prompt normalization, and Rust's root-level image then subcommand image ordering. Full Rust session startup side effects, tool router construction, skills injection, and configured environment expansion remain pending.

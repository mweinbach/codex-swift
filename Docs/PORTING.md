...
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled remote plugin detail reads
  - `plugin/read` now preserves Rust's exactly-one-source validation for `marketplacePath` vs. `remoteMarketplaceName`, validates absolute local marketplace paths, returns Rust-shaped local plugin detail for local marketplace manifests, and returns the Rust disabled-feature error for remote plugin reads; `plugin/skill/read` returns the matching disabled-feature error before remote plugin-id or skill-name validation. Full local skill/hook/app/MCP detail hydration and remote catalog fetches remain pending.

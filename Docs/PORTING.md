...
- `codex-rs/app-server/src/request_processors/plugins.rs` empty plugin listing
  - `plugin/list` now accepts Rust's removed `forceRemoteSync` field, validates absolute `cwds` and known `marketplaceKinds`, returns the Rust-shaped empty plugin catalog response (`marketplaces`, `marketplaceLoadErrors`, `featuredPluginIds`) when plugin catalogs are unavailable, projects configured/local marketplace manifests with local and Git plugin summaries, interface metadata, policies, keywords, enabled-state config, and local install-state cache detection, fetches Rust's no-auth `openai-curated` featured plugin IDs from `/plugins/featured?platform=codex`, returning an empty list on fetch or decode failure, and, when `remote_plugin` is enabled with ChatGPT auth, fetches the default remote `chatgpt-global` marketplace from `/ps/plugins/list?scope=GLOBAL&limit=200` plus `/ps/plugins/installed?scope=GLOBAL`, merging installed/enabled state, remote source, policies, availability, keywords, and interface metadata. Explicit `workspace-directory` and `shared-with-me` marketplace kinds now fetch Rust's workspace remote catalogs, workspace installed state, pagination, and shared-plugin context without requiring the `remote_plugin` default-global feature flag. Plugin cache warmup and remaining remote share save/read-download-url edges remain pending.

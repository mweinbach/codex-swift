...
- 2026-05-18: extended CLI ordinary parse-failure parity for app-server
  ...
  generator missing/duplicate `--out`, proxy duplicate/missing `--sock`, and
  responses proxy missing/invalid values plus unexpected arguments.
...
- 2026-05-18: tightened CLI subcommand version runtime-oracle parity beyond
  ...
  command-specific unsupported `--version` errors, usage lines, and tips for
  the remaining non-versioned subcommands.
...
- 2026-05-18: extended CLI help runtime-oracle parity into login and Codex
  ...
  `codex cloud {exec,status,list,apply,diff} --help` output instead of
  collapsing those child help requests back to the parent command surfaces.
...
- 2026-05-18: extended CLI help runtime-oracle parity into nested sandbox and
  ...
  `codex features {list,enable,disable} --help` output instead of collapsing
  those child help requests back to the parent command surface.
...
- 2026-05-17: made the Swift port platform target explicit. The supported
  ...
  `swift build`, `swift test`, and `git diff --check`; Linux/Windows support
  remains a deliberate future portability project rather than an accidental
  partial-build promise.
...
- 2026-05-17: wired live app-server runtime notifications through the shared
  ...
  Swift live thread now fan out to every subscribed connection and still honor
  each connection's `optOutNotificationMethods`, matching Rust's
  `ThreadScopedOutgoingMessageSender` notification routing.
...
- 2026-05-17: wired live MCP tool inventory into the Swift app-server
  ...
  Rust-shaped MCP tool results to the model.
...
- 2026-05-17: attached a first Swift app-server live runtime manager to the
  ...
  prompt/tool callbacks. Subsequent slices wired MCP tool injection/refresh,
  runtime config mutation, and cross-connection runtime notification fanout.
...
- 2026-05-17: rechecked Rust commit `3de4d7f238` (`clean up instructions`).
  ...
  full-print warning stay absent while hidden nested tool declarations remain
  available in code-mode-only descriptions.
...
- 2026-05-17: rechecked Rust commit `6941f5c2c5` (`[codex] preserve
  ...
  envelopes from runtime MCP begin/end events and preserves successful MCP
  result metadata under `_meta`, not `meta`, matching Rust's exec JSONL
  projection.
...
- 2026-05-18: Swift live app-server MultiAgentV2 `send_message` and
  `followup_task` now also mirror Rust's raw missing thread-id target path:
  after resolving a raw `ThreadId` with no agent metadata, Swift emits
...
- 2026-05-18: Swift live app-server context-only `turn/start` updates now match
  ...
  non-empty live user turn consumes those pending approval/model/reasoning
  overrides and records the effective `turn_context` with the user input.
...
- 2026-05-16: ported the reusable stream-output portion of Rust commit
  ...
  parsed non-tool response items, runs contributors before hidden assistant
  markup and memory-citation fallback parsing, and preserves contributor-set
  memory citations instead of overwriting them with parsed fallback citations.
  The broader live app-server extension registry remains pending with the
  ThreadManager runtime port.
...
- 2026-05-16: rechecked Rust commit `3c6d727810` (`permissions: resolve
  ...
  `:danger-full-access` as the built-in full-access permission profile id and
  rejects the legacy `:danger-no-sandbox` alias with Rust-shaped unknown
  built-in profile errors, while retaining the full-access
  `PermissionProfile.disabled` runtime mapping.
...
- 2026-05-16: rechecked Rust commit `9c691b74d6` (`chore(config) rm
  ...
  to expose the actual `view_image` tool unconditionally like Rust's current
...
- 2026-05-16: rechecked Rust commit `7dbe1c9498` (`Remove experimental
  ...
  config key; only `model_instructions_file` loads base instructions. Config
...
- 2026-05-16: rechecked Rust commits `c25d905f61` (`permissions: support
  ...
  Rust's config reload path for explicit `runtime_workspace_roots`. Swift
  runtime config now also tracks Rust's `workspace_roots_explicit` boundary so
  profile-defined workspace roots do not count as explicit runtime roots while
...
- 2026-05-16: rechecked Rust commit `4c80435eba` (`telemetry: tag
  ...
  policy-shaped sandbox tag helper and updating sandbox metadata tests to build
  the relevant `PermissionProfile` values directly.
...
- 2026-05-16: rechecked the CLI/runtime-option portion of Rust commit
  `4e368aa2e9` (`enable/disable remote control at runtime, not via
  features`). Swift `codex app-server --remote-control` now carries a hidden
  runtime remote-control flag into executable startup, `codex remote-control`
  starts the same runtime-enabled `--listen off` path without appending
  `features.remote_control=true`, and executable startup now derives the
  initial remote-control request state from that runtime flag instead of the
  removed config feature.
- 2026-05-16: rechecked Rust commit `249d50aafc` (`Soften SQLite
...
- 2026-05-16: rechecked Rust commit `9025550709` (`app-server-protocol:
  ...
  profile works even when the config has no global `default_permissions`, and
  the selected profile's network proxy config is projected into the child
  command environment without reusing the default profile's proxy settings.
...
- 2026-05-16: rechecked the CLI portion of Rust commit `de9c5c0226` (`Fix
  ...
  provenance, config loading including `CODEX_HOME`/`cwd` and feature-flag
  enabled/override/legacy-usage detail rows, runtime provenance,
  search-command readiness, network environment readability, terminal
  environment metadata/issues, and auth credential/provider checks, update
  ...
  handshake outcome rendering, including Rust's proxy-present detail key,
  endpoint-build failure row, timeout detail, and transport/API/stream error
...
- 2026-05-16: rechecked upstream Rust commit `8543e39885` (`Preserve
  ...
  input images, reject non-Rust user-input detail values (`auto` / `low`),
  serialize app-server v2 missing image detail as Rust's explicit `null`, and
  keep explicit `original` local-image inputs on the original-resolution
  loading path instead of forcing the resized default.
...
- 2026-05-16: rechecked Rust
  ...
  `codex-rs/core/src/context_manager/history_tests.rs`. Swift context token
  estimator coverage now separately pins structured custom-tool output images
  to the fixed resized-image replacement cost and keeps text-only items at
  their raw serialized size.
- 2026-05-16: rechecked Rust
...
- 2026-05-15: rechecked Rust app-server v2 `command/exec` disabled output-cap
  and environment override round trips in
  `codex-rs/app-server-protocol/src/protocol/v2/tests.rs`. Swift protocol
  coverage now decodes the canonical `disableOutputCap` fixture and explicit
  env unset map back to the typed command params.
...
- 2026-05-15: rechecked Rust app-server v2 config experimental markers in
  ...
  guardian review config reports `config/read.approvalsReviewer` like Rust.
...
- 2026-05-15: rechecked Rust app-server v2 `fs/changed` notification
  ...
  alongside the existing watch/unwatch payload coverage.
...
- 2026-05-15: rechecked Rust app-server v2 command-exec control fixtures in
  ...
  and `command/exec/outputDelta` round-trip wire shapes from Rust's protocol
  tests.
- 2026-05-15: rechecked Rust app-server v2 `ProcessWriteStdinParams`
  serialization in `codex-rs/app-server-protocol/src/protocol/v2/tests.rs`.
  Swift now pins the close-only `process/writeStdin` control payload with
...
- 2026-05-15: rechecked Rust app-server v2 `MarketplaceAddParams`
  ...
  marketplace adds, alongside the explicit-null optional boundary.
- 2026-05-15: rechecked Rust app-server v2 `DynamicToolCallResponse`
  serialization in `codex-rs/app-server-protocol/src/protocol/v2/tests.rs`.
  Swift now pins the `contentItems` wire shape for mixed `inputText` and
  `inputImage` dynamic tool outputs, including camelCase `imageUrl`.
...
- 2026-05-15: rechecked Rust app-server v2 `PluginInterface` asset fields in
  ...
  and remote URL fields (`composerIconUrl`, `logoUrl`, `screenshotUrls`) on the
  standalone interface payload.
...
- 2026-05-15: rechecked Rust core `HookRunSummary.source` in
  ...
  while explicit `null` rejects like Rust.
...
- 2026-05-15: rechecked Rust apply-patch streaming parser fixtures in
  ...
  remaining Rust-shaped invalid update-hunk error cases.
- 2026-05-15: rechecked Rust apply-patch `parse_update_file_chunk` invalid
...
- 2026-05-15: rechecked Rust app-server feedback upload handling in
  ...
  `codex-rs/app-server/src/request_processors/feedback_processor.rs`, including
  commit `acac786d91`'s auth-derived `account_id` feedback tag. Swift
  ...
  account-id tag forwarding, and extra log-file attachment forwarding.
...
- 2026-05-15: rechecked Rust `FileSystemPermissions` in
  ...
  omitted fields default to an empty list while explicit `null` rejects.
...
- 2026-05-15: rechecked current Rust `codex-rs/app-server-protocol/src/protocol/v2/thread.rs`.
  ...
  while keeping the rest of the runtime payload intact.
...
- 2026-05-15: rechecked Rust's MCP tool schema default in
  ...
  `#[serde(default)]` `serde_json::Value` field, preserving null and arbitrary
...
- 2026-05-15: rechecked Rust's core turn item defaults in
  ...
  for `raw_content` and `completed_at_ms` while rejecting explicit `null` for
  those non-optional `#[serde(default)]` fields.
...
- 2026-05-17: rechecked Rust's symlink-aware write-path resolution and atomic
  write helpers in `codex-rs/utils/path-utils/src/lib.rs` and
  `path_utils_tests.rs`. Swift now exposes `resolveSymlinkWritePaths` with
  Rust's non-symlink, missing-target, relative-chain, and cycle fallback
  behavior, plus `writeAtomically` parent creation and replacement semantics.
...
- 2026-05-15: rechecked Rust's `ConfigRequirements`, `ManagedHooksRequirements`,
  `ConfiguredHookHandler`, `NetworkRequirements`, and related enums in
  `codex-rs/app-server-protocol/src/protocol/v2/config.rs`. Swift's shared
  app-server config protocol now carries typed wire models for
...
- 2026-05-15: rechecked Rust's model provider serde defaults in `codex-rs/model-provider-info/src/lib.rs`. Swift `ModelProviderInfo` now preserves Rust's omitted-vs-null distinction for defaulted `name`, `wire_api`, `requires_openai_auth`, and `supports_websockets` fields, and `ModelProviderInfo.createOpenAIProvider` preserves Rust's `supports_websockets = true` default for the built-in OpenAI provider while leaving OSS and Amazon Bedrock websocket support disabled.
...
- 2026-05-15: rechecked Rust's app-server request serialization table in `codex-rs/app-server-protocol/src/protocol/common.rs`. Swift already matched the remaining thread, realtime, marketplace/plugin mutation, process, command, filesystem watch, fuzzy-search session, and explicitly concurrent account rate-limit route scopes; the focused request-serialization tests now pin those Rust route-table entries directly.
...
- 2026-05-15: rechecked Rust's accepted-line repository URL canonicalization in `codex-rs/git-utils/src/info.rs`. Swift now has focused coverage that URL-like Git remotes strip query/fragment suffixes, normalize default `git` / `http` ports, preserve the Rust exact scheme allowlist, strip userinfo at the final `@`, and reject `.` / `..` owner or repo path components before accepted-line repo hashing.
- 2026-05-15: rechecked Rust's shell command parser fixtures in `codex-rs/shell-command/src/parse_command.rs`. Swift now has focused coverage for the `ag` / `ack` / `pt` / `rga` search aliases, files-with-matches alias flags, and compact `head -n50` / `tail -n+10` read summaries.
- 2026-05-16: rechecked Rust's shell command parser simplifier in `codex-rs/shell-command/src/parse_command.rs`. Swift now has focused coverage that leading `echo ...` helpers and flag-only `nl ...` helpers are dropped while preserving the primary read/search command summaries.
...
- 2026-05-16: rechecked Rust's long-lived PowerShell AST parser coverage in `codex-rs/shell-command/src/command_safety/powershell_parser.rs`. Swift command-safety tests now pin the same simple pipeline flattening contract for `Write-Output foo | Measure-Object` through the Windows PowerShell safe-command path.
- 2026-05-14: rechecked Rust commit `06e5dfa4dd` and `codex-rs/app-server-protocol/src/protocol/v2/thread.rs`. Swift `thread/fork` responses temporarily matched that revision's top-level `sessionId`; this was superseded by the 2026-05-15 current-Rust audit above.
...
- 2026-05-14: rechecked Rust's turn metadata sandbox-tag path from `codex-rs/core/src/sandbox_tags.rs` and `codex-rs/core/src/turn_metadata.rs`. Swift now derives the turn metadata `sandbox` value from `PermissionProfile`, platform sandbox availability, Windows sandbox level, and managed-network enforcement like Rust, and exposes Rust's closest legacy `sandbox_policy` label helper for future tool-dispatch metrics, with focused coverage for disabled, external, unrestricted managed, root-write managed, read-only platform tags, workspace-write policy projection, and explicit subagent thread-source metadata.
...
- 2026-05-14: rechecked Rust commit `aadcae9f3c` and `codex-rs/models-manager/models.json`. Swift's bundled model catalog already mirrors the current `gpt-5.5` / `gpt-5.4` service-tier metadata; coverage now pins the medium default reasoning effort and the current `priority` tier description for both models.
- 2026-05-15: rechecked Rust's bundled model prompts in `codex-rs/models-manager/prompt.md` and `codex-rs/core/{gpt_5_codex_prompt.md,gpt_5_1_prompt.md,gpt_5_2_prompt.md,gpt-5.1-codex-max_prompt.md,gpt-5.2-codex_prompt.md}`. Swift model prompt resources now match Rust's removal of the legacy standalone sandbox/approval primer; sandbox and approval guidance stays injected through the runtime permission-instructions path instead of duplicated in base prompts.
- 2026-05-15: rechecked Rust's compact summary prefix resource in `codex-rs/core/templates/compact/summary_prefix.md`. Swift now preserves the no-trailing-newline resource boundary so summary detection uses exactly one separator newline like Rust's `include_str!` template.
- 2026-05-15: rechecked Rust commit `61142b6169` and `codex-rs/core/src/tools/tool_search_entry.rs`. Swift deferred MCP tool-search entries now sort by Rust's structural tool identity `(namespace, name)` instead of flattened canonical strings, preserving the post-`ToolName::display()` ordering boundary.
...
- 2026-05-14: rechecked Rust's app-server v2 `SandboxPolicy` projection from `codex-rs/app-server-protocol/src/protocol/v2/permissions.rs` and `thread/start` / `thread/resume` / `thread/fork` / `turn/start` payloads. Swift typed app-server turn/thread surfaces and live thread lifecycle responses now encode sandbox policies with Rust's camelCase app-server tags and fields instead of the core legacy kebab/snake sandbox persistence shape.
...
- 2026-05-14: rechecked Rust's app-server v2 `CodexErrorInfo` projection from `codex-rs/app-server-protocol/src/protocol/v2/shared.rs`. Swift app-server `TurnError.codexErrorInfo` now encodes and decodes the Rust v2 camelCase unit/object variants while preserving the separate snake_case core rollout/event `CodexErrorInfo` shape.
- 2026-05-14: rechecked Rust's app-server v2 `PluginHookSummary` from `codex-rs/app-server-protocol/src/protocol/v2/plugin.rs` and `HookEventName` from `protocol/v2/hook.rs`. Swift typed plugin details now encode hook event names with Rust's app-server camelCase enum values instead of the core snake_case hook enum.
- 2026-05-14: rechecked Rust's app-server v2 item lifecycle and raw-response notification payloads from `codex-rs/app-server-protocol/src/protocol/v2/item.rs`. Swift shared protocol models now carry typed `item/started`, `item/completed`, `rawResponseItem/completed`, and `agentMessageDelta` payloads with Rust's camelCase timing and item identifiers.
...
- 2026-05-17: rechecked Rust commit `d18a7c982e` (`chore(config) rm Feature::CodexGitCommit`). Swift now treats the removed `codex_git_commit` flag and deleted `commit_attribution` setting as inert for prompt assembly, so non-interactive and debug prompt-input paths no longer inject commit-message trailer instructions even if legacy config tries to enable them.
...
- 2026-05-15: rechecked Rust's app-server `CodexErrorInfo` projection from `codex-rs/app-server-protocol/src/protocol/v2/tests.rs`. Swift now has focused notification coverage that app-server error payloads use Rust's camelCase `cyberPolicy`, `responseTooManyFailedAttempts.httpStatusCode`, and `activeTurnNotSteerable.turnKind` forms while core protocol persistence keeps its separate snake_case shape.
...
- 2026-05-14: rechecked Rust commit `001363188a` and `codex-rs/core-plugins/src/lib.rs`. Swift now has focused end-to-end coverage that the `request_plugin_install` tool description advertises the allowlisted `openai-developers@openai-curated` plugin entry when it is discoverable and not already installed.
- 2026-05-14: rechecked Rust's legacy notify hook in `codex-rs/hooks/src/legacy_notify.rs`. Swift now builds legacy notification subprocesses with stdin, stdout, and stderr redirected to null like Rust's fire-and-forget `notify_hook`.
...
- 2026-05-15: rechecked Rust's external-agent plugin migration detail fixtures in `codex-rs/app-server-protocol/src/protocol/v2/tests.rs`. Swift now has focused protocol coverage that `PLUGINS` migration items and import params decode the legacy `details.plugins[]` shape with `marketplaceName` and `pluginNames`, while preserving the Rust default-empty detail arrays for omitted sibling detail fields.
...
- 2026-05-15: rechecked Rust commit `ae15343243` and plugin-share protocol models from `codex-rs/app-server-protocol/src/protocol/v2/plugin.rs`. Swift already had route behavior for plugin share save/update/list/delete; the shared protocol model layer now carries Rust's `PluginShareSaveParams`, save/update/delete response shapes, target/principal enums, and update-target payloads, including Rust's explicit-null encoding for optional share-save fields, required update-target discoverability response field, and the closed update discoverability enum that rejects `LISTED` like Rust's `PluginShareUpdateDiscoverability`.
- 2026-05-16: pinned Rust commit `ae15343243`'s unlisted workspace-link behavior for `plugin/share/save`: Swift now has focused coverage that `discoverability: "UNLISTED"` adds the current ChatGPT account as a workspace principal even when the client omits `shareTargets`.
...
- 2026-05-14: rechecked Rust's app-server v2 memory-citation projection for `ThreadItem::AgentMessage`. Swift now exposes the typed app-server `memoryCitation` payload with Rust's `threadIds` key while keeping rollout persistence on its separate `rolloutIds` shape, and preserves Rust's explicit-null `phase`/`memoryCitation` defaults when an agent message omits them.
...
- 2026-05-14: rechecked Rust's app-server v2 `ThreadItem::Plan` and `ThreadItem::Reasoning` shapes from `codex-rs/app-server-protocol/src/protocol/v2/item.rs`. Swift shared thread-item models now have focused coverage for the `plan` text payload, `reasoning` summary/content arrays, and Rust's default empty reasoning arrays when those fields are omitted.
...
- 2026-05-14: rechecked Rust's app-server v2 `ThreadItem::DynamicToolCall` shape from `codex-rs/app-server-protocol/src/protocol/v2/item.rs`. Swift shared thread-item models now carry the typed dynamic-tool-call item variant with explicit-null namespace/content/success/duration fields, app-server camelCase status values, JSON arguments, and typed `inputText`/`inputImage` content items.
- 2026-05-14: rechecked Rust's app-server v2 `ThreadItem::CollabAgentToolCall` shape from `codex-rs/app-server-protocol/src/protocol/v2/item.rs`. Swift shared thread-item models now carry the typed collab-agent tool-call item variant with app-server camelCase tool/status/agent-state values, explicit-null prompt/model/reasoning-effort/message fields, receiver thread IDs, and `agentsStates` map payloads.
- 2026-05-14: rechecked Rust's collab-agent tool-call analytics schema and reducer from `codex-rs/analytics/src/events.rs` and `codex-rs/analytics/src/reducer.rs`. Swift now has focused reducer coverage for Rust's flattened base payload, collab tool-name mapping, sender/receiver metadata, requested model/reasoning effort, agent-state/completed/failed counts, completed/failed terminal mapping, and missing-start/double-completion/in-progress suppression.
- 2026-05-14: rechecked Rust's `TrackEventsRequest`, untagged `TrackEventRequest`, and `track_event_request_batches` helper from `codex-rs/analytics/src/events.rs` and `codex-rs/analytics/src/client.rs`. Swift now has a supported-event upload envelope for compaction, turn lifecycle, turn-steer, command/file/MCP/dynamic/collab/web/image tool analytics, accepted-line fingerprints, and review events, preserving Rust's untagged event JSON and accepted-line isolated-request batching rule.
- 2026-05-16: rechecked Rust commit `3444b0d60a` and the review analytics schema in `codex-rs/analytics/src/events.rs`. Swift review analytics now pins the schema-only `codex_review_event` family, including review subject/reviewer/trigger/status/resolution enum values, explicit-null thread/turn/item lineage fields, and typed `ThreadSource` encoding on tool-item bases.
- 2026-05-14: rechecked Rust's `send_track_events_request` transport from `codex-rs/analytics/src/client.rs`. Swift now has a general analytics uploader for `CodexTrackEventsRequest` that preserves Rust's ChatGPT-token-only upload gate, API-key no-op behavior, `/codex/analytics-events/events` endpoint, bearer/account headers, JSON body, and 10-second timeout.
- 2026-05-14: rechecked Rust's app-server notification analytics feed from `codex-rs/analytics/src/client.rs` and `reducer.rs`. Swift now has a live tool-item analytics client that ingests app-server `item/started` and `item/completed` notifications, runs the command/file/MCP/dynamic/collab/web/image reducers, batches with the Rust accepted-line isolation rule, and uploads through the shared `CodexTrackEventsRequest` transport from the app-server runtime path.
- 2026-05-14: rechecked Rust's `ThreadMetadataState::from_thread_metadata` analytics provenance from `codex-rs/analytics/src/reducer.rs` and `events.rs`. Swift live tool-item analytics now preserve loaded-thread `thread_source`, subagent source names, and thread-spawn parent thread IDs when app-server runtime item notifications become uploaded analytics events.
- 2026-05-14: rechecked Rust's `track_compaction` custom analytics fact from `codex-rs/analytics/src/client.rs`, `facts.rs`, `events.rs`, and `reducer.rs`. Swift now has a focused compaction analytics fact/reducer path plus uploader-facing client method that emits `codex_compaction_event` payloads with Rust-shaped app-server/runtime/thread provenance.
- 2026-05-14: rechecked Rust's `CodexTurnEventParams` analytics shape from `codex-rs/analytics/src/events.rs`, `facts.rs`, and `reducer.rs`. Swift now has Rust-shaped `codex_turn_event` models, explicit-null optional fields, turn initialization/submission/status enums, token/timing projection, reserved tool-count null projection, focused reducer coverage, an upload-facing client method, and live app-server runtime ingestion for `turn/start`, token-count, error, and terminal turn events.
- 2026-05-14: rechecked Rust's `CodexTurnSteerEventParams` analytics shape from `codex-rs/analytics/src/facts.rs`, `events.rs`, and `reducer.rs`. Swift now has Rust-shaped `codex_turn_steer_event` models, explicit-null accepted/expected/rejection fields, thread provenance projection, focused reducer coverage, an upload-facing client method, and live app-server accepted/rejected `turn/steer` ingestion with image counts and Rust rejection-reason mapping.
- 2026-05-14: rechecked Rust's Guardian review custom analytics fact from `codex-rs/analytics/src/events.rs` and `reducer.rs`. Swift now has Rust-shaped `codex_guardian_review` event models, a focused reducer, and an upload-facing client method for Guardian review facts.
- 2026-05-14: rechecked Rust's `GuardianReviewTrackContext` and `GuardianReviewAnalyticsResult` helpers from `codex-rs/analytics/src/events.rs` and `client.rs`. Swift now builds Guardian review analytics facts from the Rust-shaped tracking context/result split, including `without_session` / `from_session` defaults, timing projection, token-usage projection, and an upload-facing client overload.
...
- 2026-05-15: rechecked Rust commit `5b87bd2845` (thread TUI naming). Swift has no current TUI status-line or terminal-title renderer counterpart for Rust's thread-title fallback to thread id, but config loading now pins the new canonical `thread-id` status/title item identifier that Rust emits while preserving Swift's raw string-array config surface.
...
- `codex-rs/cli/src/desktop_app/windows.rs`
  - executable `codex app [PATH] [--download-url URL]` now has Swift coverage for the Rust Windows branch, including Start menu AppID discovery through PowerShell, Explorer shell-target launch for installed Codex Desktop apps, installer URL opening, Microsoft Store fallback when the default installer open fails, override URL failure handling, and Rust's Windows extended-path display normalization for `\\?\` and `\\?\UNC\` workspace paths.
...
- `codex-rs/exec/src/event_processor_with_human_output.rs` final-message recovery
  ...
  - rechecked Rust commit `8adb6032cc` and the sandbox-summary helper. Swift startup config summaries now derive the sandbox line from the effective permission profile when available, show runtime workspace roots such as `--add-dir`, and keep internal writable roots out of the human summary.
...
- `codex-rs/core/src/function_tool.rs`
  - function-call error variants and Rust `thiserror` display strings for respond-to-model and fatal errors, plus Rust registry messages for unsupported function/custom tool calls
...
- `codex-rs/analytics/src/events.rs` review analytics schema
  - review-event metadata, subject/reviewer/trigger/status/resolution enums, timing fields, and nullable thread/item lineage now have Swift `Codable` wire-shape coverage. Swift also has a focused review analytics reducer/client upload method that enriches review facts with runtime/app-server context and emits Rust's `codex_review_event` request shape. Live runtime review-event source wiring remains pending with the broader analytics event pipeline.
- `codex-rs/analytics/src/events.rs` command-execution tool item analytics schema from Rust commit `fbdbc6b2fe`
  - command execution tool-item analytics now have Swift wire-shape coverage for the flattened base payload, app-server/runtime/thread provenance, lifecycle and execution duration fields, review counters, final approval/terminal/failure enums, command execution source values, exit code, and command-action count fields. Swift now also has a focused command-execution item lifecycle reducer that tracks item starts, emits Rust-shaped completion analytics, maps agent/user-shell/unified-exec tool names and terminal statuses like Rust, and suppresses missing-start, double-completion, and in-progress completions. The live app-server runtime path now feeds `item/started` and `item/completed` notifications into this reducer and uploads the resulting events with Rust-shaped thread/subagent provenance.
- `codex-rs/analytics/src/events.rs` file-change tool item analytics schema from Rust commit `fbdbc6b2fe`
  - file-change tool-item analytics now have Swift wire-shape coverage for Rust's flattened base payload, `apply_patch` tool name, file change/add/update/delete/move counts, item lifecycle duration, patch terminal/failure status mapping, and missing-start/double-completion/in-progress suppression in a focused reducer. The live app-server runtime path now feeds item lifecycle notifications into this reducer and uploads the resulting events.
- `codex-rs/analytics/src/events.rs` MCP tool-call analytics schema from Rust commit `fbdbc6b2fe`
  - MCP tool-call analytics now have Swift wire-shape coverage for Rust's flattened base payload, MCP server/tool names, error-presence flag, execution duration, completed/failed terminal mapping, and missing-start/double-completion/in-progress suppression in a focused reducer. The live app-server runtime path now feeds item lifecycle notifications into this reducer and uploads the resulting events.
- `codex-rs/analytics/src/events.rs` dynamic tool-call analytics schema from Rust commit `fbdbc6b2fe`
  - Dynamic tool-call analytics now have Swift wire-shape coverage for Rust's flattened base payload, dynamic tool name, success flag, output content/text/image counts, execution duration, completed/failed terminal mapping, and missing-start/double-completion/in-progress suppression in a focused reducer. The live app-server runtime path now feeds item lifecycle notifications into this reducer and uploads the resulting events.
- `codex-rs/analytics/src/events.rs` web-search analytics schema from Rust commit `fbdbc6b2fe`
  - Web-search analytics now have Swift wire-shape coverage for Rust's flattened base payload, `web_search` tool name, action-kind enum values, query-present trimming, Rust query-count rules for legacy and structured search actions, completed terminal status, and missing-start/double-completion suppression in a focused reducer. The live app-server runtime path now feeds item lifecycle notifications into this reducer and uploads the resulting events.
- `codex-rs/analytics/src/events.rs` image-generation analytics schema from Rust commit `fbdbc6b2fe`
  - Image-generation analytics now have Swift wire-shape coverage for Rust's flattened base payload, `image_generation` tool name, revised-prompt and saved-path presence flags, Rust's `failed`/`error` status mapping with completed fallback for other statuses, and missing-start/double-completion suppression in a focused reducer. The live app-server runtime path now feeds item lifecycle notifications into this reducer and uploads the resulting events.
- `codex-rs/analytics/src/events.rs` compaction analytics schema
  - compaction analytics now have Swift wire-shape coverage for Rust's event params, enum string values, explicit-null provenance/error/duration fields, token-count fields, and timestamp fields. Swift also has a focused compaction analytics fact/reducer and client upload method for Rust's `track_compaction` custom fact. Live runtime compaction instrumentation remains pending with the broader compaction runtime.
- `codex-rs/analytics/src/events.rs` turn analytics schema
  - turn analytics now have Swift wire-shape coverage for Rust's `codex_turn_event` payload, submission type, initialization mode, terminal status values, explicit-null optional fields, token-usage fields, started/completed/duration timestamps, error info, and app-server/runtime/thread provenance. Swift also has a focused reducer/client upload method that preserves Rust's current reserved tool-count nulls, and the live app-server runtime path now records accepted `turn/start` requests, token-count updates, error facts, and terminal turn events before uploading a Rust-shaped `codex_turn_event`.
- `codex-rs/analytics/src/events.rs` turn-steer analytics schema
  - turn-steer analytics now have Swift wire-shape coverage for Rust's `codex_turn_steer_event` payload, accepted/rejected result values, rejection-reason values, explicit-null expected/accepted/rejection fields, image counts, created-at timestamps, and thread provenance. Swift also has a focused reducer/client upload method plus live app-server `turn/steer` ingestion for accepted steers and rejected no-active/mismatched/empty/oversized-input outcomes.
- `codex-rs/analytics/src/events.rs` Guardian review analytics custom fact
  - Guardian review analytics now have Swift wire-shape coverage for Rust's flattened `codex_guardian_review` payload, network-access and MCP reviewed-action projection, explicit-null optional review/session/token fields, `GuardianReviewTrackContext` / `GuardianReviewAnalyticsResult` fact construction, `without_session` / `from_session` defaults, timing and token-usage projection, and focused reducer/client upload methods for Rust's `track_guardian_review` custom fact. Live Guardian runtime instrumentation remains pending.
...
- `codex-rs/core/src/tools/context.rs` code-mode result conversion
  - Swift now mirrors Rust's `response_input_to_code_mode_result` boundary for code-mode nested tool consumers: message/function/custom outputs collapse nonblank text and image URLs into newline-joined strings, tool-search output stays a JSON array, and MCP tool-call output exposes the raw `CallToolResult` JSON instead of the model-facing function-call output string.
...
- `codex-rs/protocol/src/protocol.rs` session and collab agent metadata
  - tightened Swift `agent_role` / legacy `agent_type` alias decoding for session metadata, thread-spawn session sources, and collab agent refs/statuses so duplicate spellings reject like Rust serde instead of silently preferring one value.
- `codex-rs/core/src/tools/handlers/agent_jobs_spec.rs`
  - added Swift Responses tool specs for `spawn_agents_on_csv` and `report_agent_job_result`, preserving Rust names, descriptions, required fields, loose object schemas for `output_schema`/`result`, explicit supported-tool exposure, and Rust's `enable_fanout` tool gating that exposes `report_agent_job_result` only to `agent_job:<job_id>` worker subagents. A shared Swift agent-job tool executor now owns the Rust-shaped handler behavior for session/runtime callers and is registered for live app-server turns.
- `codex-rs/state/src/model/agent_job.rs` and `codex-rs/state/src/runtime/agent_jobs.rs`
  - added Swift SQLite agent-job and job-item persistence, including Rust-matching status strings, creation schema, row-order listing with status/limit filters, job lifecycle updates, item transition guards, thread-scoped atomic result reporting, and progress counts. Live app-server CSV fanout now opens these tables from the shared state database.
- `codex-rs/core/src/tools/handlers/agent_jobs.rs` helper layer
  - added Swift CSV parsing, duplicate-header validation, row-to-job-item construction with `id_column` deduping, instruction-template rendering, default output path derivation, CSV escaping, and output CSV snapshot rendering for agent jobs. Live app-server turns now dispatch these handlers through the shared executor.
- `codex-rs/core/src/tools/handlers/agent_jobs.rs` runtime helper layer
  - added Swift concurrency/runtime normalization, runner option validation for thread-spawn depth and zero thread limits, child spawn-config snapshot construction/carrying from live turn fields, worker prompt rendering, spawn-result/failure-summary wire models, `report_agent_job_result` result wire shape, `spawn_agents_on_csv` job preparation including Rust's pending-job failure transition when runner options reject, missing-output CSV snapshot export with parent-directory creation, final result assembly with the first five failed item summaries, runner recovery/finalization helpers for stale running items, malformed assigned thread IDs, final worker status, and active item timeout reaping, plus pending-item spawn-slot handling for prompt dispatch, Rust's `agent_job:<job_id>` subagent source marker, inherited environment selections, agent-limit backoff, spawn failures, assignment races, worker shutdown, and loop orchestration for spawn/reap/finalize/idle-wait, cancellation, auto-export failure, and completion transitions. `spawn_agents_on_csv` now has shared function-tool execution with injected state store, worker runner callbacks, parent-derived spawn config, environment selections, and depth/thread-limit gates, including Rust-shaped JSON results and missing-runner rejection; non-interactive and live app-server turns both delegate to the same handler. Full Rust worker stop propagation beyond store cancellation remains pending.
  - tightened `spawn_agents_on_csv` environment validation so Swift now preserves Rust's distinct rejection for a single remote turn environment (`spawn_agents_on_csv is not supported for remote environments`) while keeping the existing `spawn_agents_on_csv requires exactly one local environment` response for zero or multiple selections. Live app-server registration passes the remote environment ID set into the shared handler.
- `codex-rs/core/src/tools/handlers/agent_jobs/report_agent_job_result.rs`
  - added Swift `report_agent_job_result` argument decoding, object-only result validation, store-backed thread-scoped acceptance, Rust-shaped `{ "accepted": bool }` responses, accepted `stop: true` cancellation with the Rust error message, and shared function-tool execution that preserves Rust's worker-only registration semantics by rejecting direct dispatch unless the session source is an `agent_job:<job_id>` subagent. Live app-server worker subagents now inherit that registration through their `agent_job:<job_id>` session source.
- `codex-rs/core/src/tools/handlers/agent_jobs.rs` live app-server registration
  - wired live app-server function-tool dispatch through the shared Swift agent-job executor, opened the agent-job tables on the same `state_5.sqlite` database as live thread graph state, and added live worker spawning with Rust's `agent_job:<job_id>` subagent session metadata, inherited spawn-config snapshots, environment selections, worker status polling, shutdown cancellation, and idle waits. Full Rust `AgentControl` parity remains pending for broader session-management behavior outside the CSV fanout path.
...
- `codex-rs/app-server/tests/suite/v2/safety_check_downgrade.rs` cyber-policy error projection
  - pinned Swift app-server runtime notification coverage for Rust's `cyber_policy` failure path, including `codexErrorInfo: "cyberPolicy"`, `willRetry: false`, explicit-null `additionalDetails`, and the accompanying system-error status transition.
...
- `codex-rs/config/src/config_toml.rs` and `codex-rs/core/src/config/mod.rs`
  ...
  - Swift runtime config now loads Rust's top-level `review_model`, `disable_paste_burst`, `[analytics].enabled` with profile override, and `[feedback].enabled` with Rust's default-enabled feedback behavior.
  - Swift runtime config now loads Rust's `[otel]` defaults and trace metadata, including `log_user_prompt`, `environment`, exporter selectors, default Statsig metrics export, `span_attributes`, nested `tracestate` member fields, invalid trace-metadata filtering, Rust-shaped startup warnings, and the shared provider-startup preflight that validates span attributes only when traces are exported while always validating configured tracestate before provider setup. Swift also mirrors Rust's app-server metrics exporter gate where `[analytics].enabled` overrides `--analytics-default-enabled` and an omitted analytics setting falls back to the app-server default flag. Full OpenTelemetry provider/exporter runtime installation remains pending.
  ...
  - `debug.config_lockfile` now loads Rust's nested export/load paths, version-mismatch override, model-catalog resolved-field save toggle, relative path resolution, inline table form, CLI override form, and default values. The Swift port also reads config lockfiles, validates lock metadata, removes lock-control debug tables before comparison, reports Rust-shaped version-mismatch/config-diff errors, retains `configLockToml` when `load_path` is configured, replays the runtime config from the lockfile config while carrying the original debug replay settings forward like Rust, and has the Rust-shaped lockfile TOML export primitive for `<thread_id>.config.lock.toml` with parent-directory creation. App-server `thread/start` now writes the configured lockfile export for new Swift threads, using the materialized thread id, stripped lock-control and non-replayable input config, resolved model provider, web-search mode, plan-mode reasoning effort, verbosity, instruction-context booleans, background terminal timeout, canonical feature states and special feature configs, resolved memory settings, agent limits, and skill-instruction inclusion even when model-catalog field saving is disabled, and resolved thread-start session fields for model, approval policy/reviewer, and service tier when that Rust toggle is enabled; broader live-session resolved-field export and replay validation remain pending with the live session config-lock runtime.
...
- `codex-rs/config/src/mcp_edit_tests.rs` MCP config replacement fields
  - Swift MCP config parsing/replacement now preserves Rust's `required`, `supports_parallel_tool_calls`, `default_tools_approval_mode`, OAuth scopes/resource, nested `oauth.client_id`, and per-tool approval-mode override fields, including the empty parent `[mcp_servers.<name>.tools]` table Rust writes before sorted per-tool tables.
- `codex-rs/app-server/src/request_processors/mcp_processor.rs` and `codex-rs/rmcp-client/src/perform_oauth_login.rs` MCP OAuth login scopes/resource handling
  - `mcpServer/oauth/login` and `codex mcp login` now pass configured MCP OAuth scopes when request scopes are omitted, preserve explicit request scopes precedence, reject malformed `scopes` and `timeoutSecs` values with Rust protocol decode errors, propagate `oauth_resource` into the authorization URL as Rust's `resource` query parameter, pass configured `oauth.client_id` into the public-client authorization/token flow instead of dynamic registration, use normalized discovered `scopes_supported` metadata when no explicit or configured scopes are present, parse OAuth provider callback errors with Rust-shaped messages, and retry the CLI login flow without scopes when a provider rejects discovered scopes.
...
- `codex-rs/core/src/session/session.rs`, `codex-rs/exec/tests/suite/mcp_required_exit.rs`, `codex-rs/app-server/tests/suite/v2/thread_start.rs`, `codex-rs/app-server/tests/suite/v2/thread_resume.rs`, and `codex-rs/app-server/src/request_processors/turn_processor.rs` required MCP startup failures and approval-reviewer overrides
  - `thread/start`, `thread/resume`, `thread/fork`, and non-interactive exec now reject enabled required stdio MCP servers whose command cannot be resolved before creating or reopening a rollout, preserving Rust's sorted `required MCP servers failed to initialize: <server>: <error>` failure shape for broken-command startup. Loaded Swift app-server threads now also start enabled MCP servers through the live MCP manager slice, preserve tool inventory for optional servers, ignore optional startup failures, emit Rust-shaped `mcpServer/startupStatus/updated` starting/ready/failed notifications from live startup, and wait only on required MCP failures like Rust's session connection manager. Thread lifecycle and `turn/start` approval-reviewer overrides now reject unknown enum values instead of falling back to the configured reviewer. Running-thread tool routing remains pending with the broader MCP manager port.
...
- Initial `codex-rs/app-server` rollout-listing surface
  ...
  - `command/exec` process-id sessions and `process/spawn` now bound post-exit stdout/stderr draining to Rust's two-second `IO_DRAIN_TIMEOUT_MS`, so inherited pipes held by descendant processes no longer delay the terminal response or `process/exited` notification until those descendants close stdio.
  - `command/exec`, `process/spawn`, and exec-server `process/terminate` now supervise spawned process trees and signal tracked descendants during explicit termination, process exit, session shutdown, and stdio-client teardown so shell-spawned background children do not outlive the owning live session.
  - `modelProvider/capabilities/read`, `collaborationMode/list`, `thread/realtime/listVoices`, `plugin/share/list`, and `config/mcpServer/reload` now preserve Rust's required empty-struct params contract, accepting `{}` while rejecting omitted, `null`, or scalar params with Rust-shaped invalid-request errors before feature gates, auth checks, or route logic.
  - `memory/reset`, `windowsSandbox/readiness`, `account/logout`, and `account/rateLimits/read` now preserve Rust's `Option<()>` params contract, accepting omitted or explicit `null` params while rejecting `{}` with the Rust-shaped `expected unit` invalid-request error before route-specific auth, capability, or runtime checks.
  - `configRequirements/read` now preserves Rust's `Option<()>` params contract, accepting omitted or explicit `null` params while rejecting `{}` with the Rust-shaped `expected unit` invalid-request error.
  ...
  - legacy `gitDiffToRemote` now validates `cwd` through Rust's v1 `PathBuf` parameter shape, rejecting missing, null, and non-string values with Rust-shaped invalid-request errors before attempting git inspection.
  - legacy `loginApiKey` now validates the Rust v1 required `apiKey` string before mutating auth state, preserving missing/null/non-string parameter errors.
  ...
  - app-server `initialize` now requires Rust's typed `clientInfo` object before mutating initialized state, including required string `name` and `version` fields plus optional/null string `title`, instead of accepting missing or malformed client metadata.
  ...
  - app-server `initialize` now emits Rust-shaped `configWarning` notifications for configured process warnings, Swift config-loader startup warnings, and invalid startup config loads, preserving explicit `details: null`, optional `path`/`range` omission, text range projection, and generic client notification opt-out filtering. After an invalid startup config, initialized `model/list` and `experimentalFeature/list` calls now keep serving from a default runtime config like Rust instead of re-reading the invalid user file; broader shared fallback coverage for all reload paths remains pending.
...
- `codex-rs/app-server/tests/suite/conversation_summary.rs` relative rollout path lookup
  ...
  - legacy `getConversationSummary` params now preserve Rust's v1 untagged enum decoding for `rolloutPath` and `conversationId`, rejecting missing fields, malformed values, and snake_case aliases with the Rust-shaped invalid-request message instead of falling through to a missing-id error.
...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` experimental thread history pagination
  ...
  - `thread/turns/list` and `thread/turns/items/list` now preserve Rust's explicit concurrent request serialization contract (`serialization: None`) instead of joining the mutating per-thread request queue.
...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` thread status notifications
  ...
  - rechecked current Rust `codex-rs/app-server/src/bespoke_event_handling.rs`: Swift live network approval requests now use Rust's network presentation by sending `networkApprovalContext` while omitting command, cwd, and commandActions from `item/commandExecution/requestApproval`; command approval requests still include the command presentation.
...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` thread name updates
  - `thread/name/set` now validates Rust-shaped typed `threadId` and `name` params, trims names, rejects empty names and missing rollouts, updates configured SQLite thread metadata titles, appends Rust-compatible `session_index.jsonl` entries, emits `thread/name/updated`, and broadcasts that notification to other initialized app-server connections for both loaded and not-loaded stored threads. `thread/read` and `thread/resume` now apply the configured state-store title overlay before legacy `session_index.jsonl` names, matching Rust's ThreadStore-backed name attachment.
...
- `codex-rs/app-server/src/request_processors/mcp_processor.rs` MCP resource/tool call validation
  - `mcpServer/resource/read` now preserves Rust thread-id parsing and missing-thread errors before live MCP dispatch; no-thread reads call effective configured streamable HTTP MCP servers, including the host-owned `codex_apps` server under ChatGPT auth, with header/env/bearer-token projection and session-id propagation plus configured stdio MCP servers with command/env/cwd projection, and materialized-thread stdio resource reads use the rollout cwd when the MCP server config omits `cwd`. `mcpServer/tool/call` now validates request fields before requiring a loaded Swift app-server thread like Rust's `ThreadManager` route, rejects unloaded/materialized-only thread ids with Rust-shaped `thread not found`, injects Rust's `threadId` `_meta`, and calls effective configured streamable HTTP/stdio MCP servers plus the product-owned built-in `memories` MCP server for loaded Swift threads, including host-owned `codex_apps`; configured stdio tool calls use the loaded rollout cwd when the MCP server config omits `cwd`. Live runtime MCP tool-call begin/end events now project through Rust-shaped item lifecycle notifications.
...
- `codex-rs/core/src/mcp_tool_call.rs` live MCP tool-call lifecycle projection
  - Swift app-server now projects live `mcp_tool_call_begin` and
...
- `codex-rs/app-server/src/request_processors/windows_sandbox_processor.rs` Windows sandbox setup start
  - `windowsSandbox/setupStart` now validates setup mode and optional absolute `cwd` with Rust-shaped enum/string request decoding, returns Rust's immediate `{ started: true }` response, emits the Rust-shaped `windowsSandbox/setupCompleted` notification with the non-Windows setup error, and the app-server route now has injectable success-path coverage proving Rust's successful-setup config persistence semantics: it passes the requested cwd/profile into setup, writes the selected `[windows].sandbox` mode, and clears legacy Windows sandbox feature flags at the active profile scope when setup succeeds. Real native Windows setup execution remains pending.
- `codex-rs/app-server/src/request_processors/mcp_processor.rs` MCP config reload
  - `config/mcpServer/reload` now accepts only Rust's required empty-object params shape, reloads configured MCP servers from the current layered config including CLI/session flags and managed config, reports config reload failures with the Rust route-level internal error prefix, queues Rust-shaped `refresh_mcp_servers` config snapshots for loaded Swift app-server threads using each loaded thread's cwd/project-layer snapshot, submits Rust-shaped `Op::RefreshMcpServers` values when a live runtime submitter is attached, refreshes the running Swift live MCP manager's enabled-server startup state for loaded threads, propagates submitter failures through the route-level MCP refresh error, and returns Rust's empty response object. Full connection-manager lifecycle notifications and running-thread tool routing remain pending with the broader live-thread manager port.
...
- `codex-rs/app-server/src/request_processors/account_processor.rs` external ChatGPT auth tokens
  ...
  - `account/login/start` v2 API-key and external ChatGPT token variants now preserve Rust's required/optional string decoding for `type`, `apiKey`, `accessToken`, `chatgptAccountId`, and `chatgptPlanType`, rejecting missing required fields and malformed string fields before auth state changes.
- `codex-rs/app-server/src/request_processors/account_processor.rs` managed ChatGPT auth refresh
  ...
  - `account/read` route decoding now preserves Rust's v2 `GetAccountParams` defaulted bool behavior for `refreshToken`: omitted values default to `false`, while explicit `null`, strings, and numeric values are rejected before auth refresh work.
  - legacy `getAuthStatus` route decoding now preserves Rust's v1 `Option<bool>` behavior for `includeToken` and `refreshToken`: omitted and explicit `null` values default to no token inclusion or forced refresh, while strings and numeric values are rejected before auth refresh work.
- `codex-rs/app-server/src/request_processors/account_processor.rs` ChatGPT browser login
  ...
  - `account/login/start` ChatGPT browser route decoding now preserves Rust's v2 defaulted bool behavior for `codexStreamlinedLogin`: omitted values default to `false`, while explicit `null`, strings, and numeric values are rejected before browser-login setup work.
- `codex-rs/app-server/src/request_processors/account_processor.rs` ChatGPT device-code login
  - `account/login/start` now accepts the Rust `chatgptDeviceCode` variant, requests a user code from the ChatGPT issuer (including the app-server issuer override used by Rust tests), accepts Rust's legacy `usercode` response alias while rejecting duplicate `user_code`/`usercode` fields, preserves Rust's device-code `interval` response decoding where an omitted interval defaults to zero but explicit `null` is rejected, returns `loginId`, `verificationUrl`, and `userCode`, completes polling/token exchange in the background, emits `account/login/completed` and successful `account/updated` notifications, persists ChatGPT tokens, queues Rust-shaped MCP refresh config snapshots for loaded Swift app-server threads after successful completion, and supports `account/login/cancel` and `account/logout` cancellation for active device-code attempts. The broader live auth-manager/cache refresh side effects remain pending with the app-server runtime port.
- `codex-rs/app-server/src/request_processors/account_processor.rs` account login replacement
  ...
  - `account/login/cancel` route decoding now preserves Rust's v2 `CancelLoginAccountParams` required string behavior for `loginId`, including missing-field and invalid-type errors before UUID parsing.
...
- `codex-rs/app-server/src/request_processors/config_processor.rs` config tools projection
  - `config/read` now projects top-level and profile-scoped `[tools]` through Rust's v2 `ToolsV2` shape, including nested `tools.web_search` `context_size`, `allowed_domains`, and `location` fields, typed `null` values for absent optional fields, and Rust's legacy behavior of ignoring boolean `tools.web_search = true`.
...
- `codex-rs/protocol/src/models.rs` MCP tool-call output input item
  - `ResponseInputItem.mcpToolCallOutput` now serializes and decodes Rust's raw `output: CallToolResult` field without the legacy Swift `Ok`/`Err` wrapper, preserves `_meta`, and converts into `ResponseItem.functionCallOutput` through Rust's `CallToolResult` payload path. Focused coverage also pins Rust's `CallToolResult` payload conversion for structured-content precedence, explicit `structuredContent: null` content fallback, `isError` success flag projection, and text-only model image sanitization before function-call output conversion.
...
- `codex-rs/core/src/tools/router.rs` client tool-search decode failures
  - Malformed client `tool_search` arguments now mirror Rust's `ToolRouter::build_tool_call` failure projection by returning a generic `function_call_output` with an empty `call_id`, omitted `success`, and a `failed to parse tool_search arguments:` message instead of a failed `tool_search_output`.
...
- `codex-rs/app-server` fuzzy file-search result projection
  ...
  - tightened legacy `fuzzyFileSearch` request decoding to match Rust's typed protocol model for required `query` / `roots` fields and malformed optional `cancellationToken` values before returning the empty-query fast path.
...
- `codex-rs/config/src/config_toml.rs` / `codex-rs/core/src/config/mod.rs` agents runtime config
  ...
  - tightened Swift agent-role layer precedence to match Rust's current `load_agent_roles`: project `.codex/agents` directories are discovered even when no sibling `.codex/config.toml` exists, declared legacy role config files may omit `developer_instructions`, and higher-precedence standalone role files can override lower declared roles while inheriting missing description/nickname metadata; duplicate standalone names inside the same discovered `agents/` directory still warn and keep the first role.
...
- `codex-rs/core/src/goals.rs` / `codex-rs/app-server/tests/suite/v2/thread_resume.rs` paused-goal resume behavior
  - rechecked Rust commit `3c2dcbef85` and pinned Swift's cold `thread/resume` goal snapshot behavior: a persisted paused goal remains `paused`, no continuation `turn/started` notification is emitted during resume, and the stored goal status is not promoted to `active`. Live running-thread goal runtime continuation/accounting remains covered by the broader pending running-thread runtime work above.
...
- `codex-rs/app-server-protocol/src/protocol/common.rs` plugin read/list request serialization
  - rechecked Rust commit `8c7a176b55` and corrected the Swift parity ledger: `plugin/list` and `plugin/read` are intentionally unscoped in Swift, so read-only plugin requests start immediately instead of waiting behind the config queue; `config/read` and `skills/list` remain shared reads while plugin/config/skill mutations keep exclusive serialization.
...
- `codex-rs/protocol/src/models.rs` MCP tool output content fallback
  - aligned Swift MCP content-block decoding with Rust's tagged content handling for function-call output conversion: unknown or missing MCP content `type` values now round-trip as raw JSON instead of shape-decoding as text/image blocks, image-bearing tool outputs stringify those unknown blocks into `input_text` fallback entries like Rust's `convert_mcp_content_to_items`, and `CallToolResult` image blocks can be sanitized for text-only models with Rust's angle-bracket placeholder while preserving content when image input is supported.
...
- `codex-rs/exec/src/exec_events.rs` MCP tool call result metadata
  - rechecked Rust commit `6941f5c2c5`; Swift already carried MCP `CallToolResult` metadata through core turn items and app-server thread item conversion, and now pins exec JSONL MCP tool-call item serialization so result metadata stays under Rust's `_meta` key and never leaks as `meta`.
...
- cross-platform product support beyond macOS, including Darwin vs
  Glibc/Musl, CryptoKit/Security replacements, non-macOS credential stores,
  Linux Landlock/Windows sandboxing, PTY/process APIs, filesystem permission
  and ACL semantics, and desktop app discovery/install behavior
...
- app-server live turn execution real ThreadManager streaming/cancellation, review execution/item completion, MCP APIs, account write-refresh flows, general config writes, and Swift-native protocol generators beyond the Rust-oracle bridge

...
- `codex-rs/app-server/src/request_processors/thread_processor.rs` manual compaction and shell command routes
  - `thread/compact/start` and `thread/shellCommand` now validate materialized thread ids, preserve the shell-command empty-string rejection, and return Rust's empty response object for accepted rollout-backed threads. Live core-op submission and progress notifications remain pending with the broader running-thread runtime.

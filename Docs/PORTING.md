...
- Initial `codex-rs/cli/src/mcp_cmd.rs` MCP command formatting
  - `codex mcp list` and `codex mcp get` now preserve Rust's disabled-server reason text in human output and `disabled_reason` JSON fields while keeping enabled servers at the Rust-shaped `null` value. Live plugin/requirements sources that compute disabled reasons remain pending with the broader MCP manager port.

...
- Initial live hook configuration and non-interactive `UserPromptSubmit` wiring from `codex-rs/hooks/src/engine/discovery.rs` and `codex-rs/core/src/hook_runtime.rs`
  - config-layer hooks now materialize executable command handlers only when hooks are enabled and the handler is managed or trusted, respect disabled hook state, preserve declaration display order, substitute `${VAR}` environment placeholders, and run trusted `UserPromptSubmit` hooks before `codex exec` submits the model prompt. Hook-emitted additional context is appended to the prompt/rollout before the model call, while `continue:false` short-circuits the non-interactive turn. Plugin hook discovery and the remaining live `SessionStart`/`PreToolUse`/`PermissionRequest`/`PostToolUse`/`Stop` attachment points remain pending.

...
- `codex-rs/app-server/src/request_processors/mcp_processor.rs` MCP resource/tool call validation
  - `mcpServer/resource/read` and `mcpServer/tool/call` now preserve Rust thread-id parsing and missing-thread errors before live MCP dispatch. Thread-backed resource reads, tool execution, and no-thread resource reads remain pending.

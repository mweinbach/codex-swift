...
- `codex-rs/app-server/src/request_processors/process_exec_processor.rs` process spawn request validation
  - `process/spawn` now rejects empty commands, empty process handles, relative `cwd`, PTY size without `tty: true`, zero terminal dimensions, and negative `timeoutMs` with Rust-shaped errors, starts non-PTY host processes for valid requests, and emits Rust-shaped `process/outputDelta` / `process/exited` notifications for buffered and streamed output. Full PTY behavior and stdin/resize control for active spawned processes remain pending.

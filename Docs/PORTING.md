...
- `codex-rs/app-server/src/command_exec.rs` command exec follow-up controls
  - `command/exec` now supports buffered and client-supplied `processId` non-PTY sessions with Rust's timeout exit code, deferred final responses for process-id sessions, `streamStdin` writes/close, `streamStdoutStderr` output-delta notifications, duplicate process-id rejection, and `command/exec/terminate`; `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` preserve Rust validation for empty stdin writes, invalid base64 input, zero terminal dimensions, no-active-process errors, and active non-PTY resize failures. `tty: true` command sessions now attach to a Darwin pseudo-terminal and accept active `command/exec/resize`; broader PTY output/control parity remains pending.

...
- `codex-rs/app-server/src/request_processors/plugins.rs` disabled plugin share/install/uninstall routes
  - `plugin/share/save`, `plugin/share/updateTargets`, `plugin/share/list`, and `plugin/share/delete` now preserve Rust's disabled sharing response when the plugins feature is unavailable; `plugin/install` validates exactly one install source and returns the remote disabled-feature error; `plugin/uninstall` accepts Rust's removed `forceRemoteSync` field, rejects malformed ids, and returns the remote disabled-feature error for remote plugin ids. Local install/uninstall and live remote mutations remain pending.

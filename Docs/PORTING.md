...
- `codex-rs/protocol/src/items.rs` plus item legacy-event projections from `codex-rs/protocol/src/protocol.rs`
  - turn item wire tags, user/hook-prompt/agent/plan/reasoning/web-search/image-generation item models, hook-prompt XML continuation parsing/serialization, item IDs, legacy message/reasoning/web-search/image-generation events, item started/completed timing fields, and item started/completed legacy bridging
...
- `codex-rs/protocol/src/protocol.rs` event envelope/tagged message model
  - Rust-compatible `Event { id, msg }` shape and `type`-tagged dispatch for currently ported event payload variants, including guardian assessments, turn lifecycle v2 aliases, and turn timing fields.

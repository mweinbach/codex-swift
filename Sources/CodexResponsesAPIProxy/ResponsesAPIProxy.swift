...
public enum ResponsesAPIProxyCommandLine {
    public enum ParsedArguments: Equatable, Sendable {
        case run(ResponsesAPIProxyOptions)
        case help
        case failure(String, Int32)
    }

    public static func parseArguments(_ arguments: [String]) -> ParsedArguments {
        var port: UInt16?
        ...
        while index < arguments.count {
            ...
            case "--dump-dir":
                guard index + 1 < arguments.count else {
                    return .failure("codex-responses-api-proxy: missing value for --dump-dir", 2)
                }
                dumpDir = URL(fileURLWithPath: arguments[index + 1])
                index += 2
            case let value where value.hasPrefix("--dump-dir="):
                dumpDir = URL(fileURLWithPath: String(value.dropFirst("--dump-dir=".count)))
                index += 1
    ...
    public static func helpText() -> String {
        ...
        Options:
          ...
          --dump-dir <DIR>       Directory where request/response dumps should be written as JSON.
          -h, --help             Print help.
        """
    }
}
